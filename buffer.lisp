...
(defun array-delete (array start length)
  "Modifies ARRAY by deleting bytes at position START for length LENGTH, and returns ARRAY.
The deleted bytes are not saved. The fill pointer for ARRAY is updated to the new, shorter length."
...
(defmethod buffer-getbyte ((buffer buffer) &optional (remove t))
  ...
  (prog1 (aref (buffer-data buffer) 0)
    (when remove
      (array-delete-at (buffer-data buffer) 1))))
